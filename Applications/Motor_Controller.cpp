...
{
GM6020_Controller::GM6020_Controller(Motors::GM6020 **Motors, PID **Vel_PIDs, PID **Pos_PIDs)
{
    this->motors   = Motors;
...
void GM6020_Controller::setTargetPosition(float *set_position, float *current_position, bool is_rad)
{
    for (int i = 0; i < LEG_MOTOR_NUM; i++)
    {
        ...
        motors[i]->setOutput(vel_PIDs[i]->operator()(
            pos_PIDs[i]->operator()(target_accumulated_positions_rad[i], current_accumulated_positions_rad[i]), motors[i]->getRPMFeedback()));
...
M3508_Controller::M3508_Controller(Motors::M3508 **Motors, PID **Vel_PIDs)
{
    this->motors   = Motors;
    this->vel_PIDs = Vel_PIDs;
...
void M3508_Controller::setTargetRPM(float *set_rpm, float *current_rpm)
{
    for (uint8_t i = 0; i < WHEEL_MOTOR_NUM; i++)
    {
        ...
        motors[i]->setOutput(vel_PIDs[i]->operator()(set_rpm[i], motors[i]->getRPMFeedback()));
